...
Okay, <Term id="typescript">TypeScript</Term> has a reputation for being confusing. I get it — the first time you see `type Props = { name: string; age: number }` your brain goes "nope." But here's the thing: TypeScript is literally just JavaScript with spell-check. Let me show you.
...
- **Your editor gets superpowers**: With TypeScript, VS Code can autocomplete property names, show you what a function expects, and underline errors as you type — like Google Docs' grammar suggestions but for code.
...
- **Refactoring without fear**: Want to rename a field from `userName` to `displayName`? TypeScript highlights every place in your entire codebase that needs to change. In JavaScript, you'd find out you missed one when a user reports a bug at 2am.
- **Job listings**: The majority of frontend job postings now require or prefer TypeScript. It's not a "nice to have" anymore — it's expected.
...
// You only need to write types when TypeScript can't guess"
  language="typescript"
  explanation="TypeScript has type inference — it's smart enough to look at the value and figure out the type on its own. If you write let name = 'Denise', TypeScript already knows it's a string. You only need to write the type explicitly when you're declaring something without a value, or when the type is ambiguous."
/>
...
This is where TypeScript really shines — describing what your data looks like:
...
// Optional parameters — add a ? to make it optional
...
<ProfileCard name='Denise' role='Creator' emoji='💜' />        // Perfect!
<ProfileCard name='Denise' />                                   // Error: missing role and emoji!
<ProfileCard name='Denise' role='Creator' emoji='💜' color='pink' /> // Error: color is not a valid prop!"
  language="tsx"
  explanation="By defining a Props type, you get autocomplete when using the component (your editor will suggest name, role, emoji, isOnline), you get error checking if you forget a required prop, AND you get documentation — anyone reading the code can see exactly what the component needs. It's a triple win."
/>
...
- TypeScript **infers** most types — you don't need to type everything
...
- Migrate gradually — you don't need to do everything at once

<Callout variant="denise-says">
TypeScript's whole job is to have your back. It's not there to make your life harder — it's there to catch the bugs you'd spend hours finding on your own. Give it a real chance (not just one frustrated afternoon) and I promise you'll wonder how you ever coded without it. Every major company uses TypeScript now, and that's not a coincidence.
</Callout>
